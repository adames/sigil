...
enum AerospaceFragment {
    ...
    /// Open / close fences. Plain ASCII, idempotent to detect: a regex
    /// match across the file picks up multi-line content between them.
    ...
    static func render(slotNames: [String]) -> String {
        ...
        lines.append("# of truth in ~/.config/workspace/spaces.json + aerospace.toml")
        lines.append("# (outside the fence), then re-run the generator.")
        lines.append("")
        ...
        if visible.isEmpty {
            ...
            lines.append("# Hyperkey collapses Caps+Shift+digit into the same chord as Caps+digit.")
            lines.append("# Use Caps+g (ws-prompt send) to send a window to a specific workspace.")
        }
        lines.append("")
        lines.append("# Cascade hook: AeroSpace's only subscribe-style event. Replaces the")
        lines.append("# yabai space_changed signal that primed ~/.cache/workspace/current.env.")
        lines.append("exec-on-workspace-change = ['/bin/bash', '-c', '\(cascadeHookPath)']")
    ...
    static func merge(block: String, into existing: String) -> String {
        ...
        guard let openRange = existing.range(of: openFence),
            ...
            // No fence found — append. Add a separator newline if the
            // file doesn't end in one.
            let needsSeparator = !existing.isEmpty && !existing.hasSuffix("\n")
            return existing + (needsSeparator ? "\n" : "") + cleanBlock + "\n"
        }

        // Extend the close-fence range to include trailing newline (if any),
        // so we don't leave a blank line behind on re-merge.
...
func cmdEmitAerospace(args: [String]) -> Int32 {
    ...
    let merged = AerospaceFragment.merge(block: block, into: existing)
