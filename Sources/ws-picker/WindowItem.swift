...
struct WindowItem: Equatable, Identifiable {
    ...
    /// Combined string used for fuzzy matching. Including the workspace
    /// name and display index lets the user reach by workspace ("code
    /// term" finds the Ghostty on the workspace named code) without
    /// juggling separate filters.
    var matchKey: String {
        let titleSegment = title.isEmpty ? "" : " \(title)"
        return "\(app)\(titleSegment) \(workspace) \(display)"
    }
