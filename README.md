...
aerospace.toml binds Caps+1..0 to the first ten workspaces (the block is
regenerated by `ws-topology emit-aerospace --write`). The author's own
aerospace.toml additionally binds Caps+n / Caps+p / Caps+Tab to
`ws-focus next|prev|last` for cycling and back-and-forth — those bindings
aren't generated or shipped, so add equivalents in the user-owned section of
your own aerospace.toml if you want them. Caps+f opens the `ws-prompt send`
overlay (digit-only); workspaces beyond ten have no digit chord — reach them
via the Caps+c window picker (`ws-picker`) or `aerospace workspace NAME` from
a shell.
